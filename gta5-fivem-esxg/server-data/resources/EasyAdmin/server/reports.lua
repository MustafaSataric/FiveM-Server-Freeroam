...
Citizen.CreateThread(function()
    ...
    if GetConvar("ea_enableCallAdminCommand", "true") == "true" then
        ...
        RegisterCommand(GetConvar("ea_callAdminCommandName", "report"), function(source, args, rawCommand)
            if args[1] then
                local time = os.time()
                local cooldowntime = GetConvarInt("ea_callAdminCooldown", 60)
                local source=source
    ...
    if GetConvar("ea_enableReportCommand", "true") == "true" then
        RegisterCommand(GetConvar("ea_reportCommandName", "calladmin"), function(source, args, rawCommand)
            if args[2] then
                local source = source
                local id = args[1]
                local valid = false
